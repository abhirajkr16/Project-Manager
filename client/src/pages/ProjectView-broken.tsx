...
export default function ProjectView() {
  ...
  const renderKanbanColumn = (status: TaskStatus, title: string) => (
    <div key={status} className="flex-1 bg-gray-50 rounded-lg p-4 min-h-[400px]">
    ...
    <div className="space-y-6">
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="flex gap-4 overflow-x-auto pb-4">
          <DroppableColumn status="todo" title="To Do">
          ...
          <DroppableColumn status="in-progress" title="In Progress">
          ...
          <DroppableColumn status="done" title="Done">
        ...
        <DragOverlay>
          {activeId ? (
            <div className="opacity-50">
              {renderTaskCard(tasks.find((t: Task) => t.id === activeId)!)}
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
